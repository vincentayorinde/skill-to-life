...
import { CvAnalysisResult, SavedResource, UserProfile } from 'types';
          ...
          <div class="grid gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-8">
              ...
              <div class="rounded-ns-lg border border-ns-border bg-ns-card p-5">
                <div class="mb-4 flex flex-col items-center text-center">
                  @if (currentUser()?.avatar) {
                    ...
                    <div
                      class="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-ns-primarySoft text-2xl font-bold text-ns-primary"
                    >
                      {{
                  ...
                  <p class="text-base font-semibold text-ns-text">
                    {{ currentUser()?.name }}
                  </p>
                ...
                <div class="mt-4 rounded-ns bg-ns-canvasSubtle p-3">
                  <p class="text-xs font-medium text-ns-muted">
                    Profile status
                  </p>
            ...
            <div class="min-w-0">
              ...
              @if (activeTab() === 'profile') {
                <div
                  class="rounded-ns-lg border border-ns-border bg-ns-card p-4 shadow-ns sm:p-6"
                >
                  <div
                    class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"
                  >
                    <div>
                      <h1 class="m-0 text-xl font-bold text-ns-text">
                        Profile
                      </h1>
                      <p class="mt-1 text-sm text-ns-muted">
                        Update your public details and sharing settings.
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div class="grid gap-4 md:grid-cols-2">
                    <div>
                      <label
                        ...
                        class="mb-1 block text-xs font-medium text-ns-muted"
                        >Username</label
                      >
                      <input
                        id="profile-username"
                  ...
                  <div
                    class="mt-6 rounded-ns border border-ns-border bg-ns-canvasSubtle p-4"
                  >
                    <div
                      class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div>
                        <p class="m-0 text-sm font-medium text-ns-text">
                          Public profile
                        </p>
                        <p class="mt-1 text-xs text-ns-muted">
                          Control whether your profile can be shared publicly.
                        </p>
                      </div>
                      <button
                        type="button"
                    ...
                    @if (profile()!.isPublic && profile()!.username) {
                      <div class="mt-4 rounded-ns bg-ns-card p-3">
                        <p class="mb-1 text-xs text-ns-muted">
                          Your public URL:
                        </p>
                        <div class="flex min-w-0 items-center gap-2">
                          <code
                            class="min-w-0 flex-1 truncate text-xs text-ns-primary"
                            >skilltolife.com/u/{{ profile()!.username }}</code
                          >
                          <button
                            type="button"
              ...
              @if (activeTab() === 'overview') {
                <div class="space-y-6">
                  ...
                  <div class="grid gap-3 sm:grid-cols-2">
                    <button
                      type="button"
                      class="rounded-ns-card border border-ns-border bg-ns-card p-4 text-left hover:border-ns-primary transition-colors"
                      (click)="activeTab.set('cv')"
                    >
                      <p class="text-sm font-semibold text-ns-text">
                        Analyse your CV →
                      </p>
                      <p class="mt-0.5 text-xs text-ns-muted">
                        Upload or paste your CV for AI career matching
                      </p>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <p class="mt-0.5 text-xs text-ns-muted">
                        {{ profile()!.savedCareers.length }} career paths
                        bookmarked
                      </p>
                    </button>
                  </div>
              ...
              @if (activeTab() === 'saved') {
                <div>
                  @if (profile()!.savedCareers.length === 0) {
                    <div
                      class="rounded-ns-lg border border-dashed border-ns-border py-16 text-center"
                    >
                      <p class="mb-2 text-base font-medium text-ns-text">
                        No saved careers yet.
                      </p>
                      <a
                        routerLink="/careers"
                        class="text-sm text-ns-primary no-underline hover:underline"
                        >Browse career paths →</a
                      >
                    </div>
                  } @else {
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      @for (
                        career of paginatedSavedCareers();
                        track career.id
                      ) {
                        <div
                          class="rounded-ns-card border border-ns-border bg-ns-card p-4"
                        >
                          <div
                          ...
                          @if (career.notes) {
                            <p class="mb-2 text-xs text-ns-muted">
                              {{ career.notes }}
                            </p>
                          }
                          <div class="flex items-center justify-between">
                            <span class="text-xs text-ns-muted">{{
                              career.savedAt | date: 'mediumDate'
                            }}</span>
                            <a
                              [routerLink]="['/careers', career.careerSlug]"
                              class="text-xs font-medium text-ns-primary no-underline hover:underline"
                              >View path →</a
                            >
                          </div>
                        </div>
                    ...
                    @if (savedCareersTotalPages() > 1) {
                      <div class="mt-6 flex items-center justify-between">
                        ...
                        <span class="text-xs text-ns-muted"
                          >Page {{ savedCareersPage() + 1 }} of
                          {{ savedCareersTotalPages() }}</span
                        >
                        <button
                          type="button"
                          class="rounded-ns border border-ns-border px-4 py-2 text-sm text-ns-muted hover:border-ns-primary hover:text-ns-text disabled:opacity-40 transition-colors"
                          [disabled]="
                            savedCareersPage() >= savedCareersTotalPages() - 1
                          "
                          (click)="savedCareersPage.set(savedCareersPage() + 1)"
                        >
                          Next →
              ...
              @if (activeTab() === 'resources') {
                <div>
                  @if (savedResourcesCount() === 0) {
                    ...
                    <div class="space-y-6">
                      @for (
                        group of paginatedResourceGroups();
                        track group.label
                      ) {
                        <div>
                          <h3
                            class="mb-3 text-sm font-semibold text-ns-muted uppercase tracking-wider"
                          >
                            {{ group.label }}
                          </h3>
                          <div class="space-y-2">
                            @for (r of group.items; track r.id) {
                              <div
                                class="flex items-center justify-between gap-4 rounded-ns border border-ns-border bg-ns-card px-4 py-3"
                              >
                                <div class="min-w-0 flex-1">
                                  <p
                                    class="truncate text-sm font-medium text-ns-text"
                                  >
                                    {{ r.resourceTitle }}
                                  </p>
                                ...
                                <div class="flex shrink-0 items-center gap-2">
                                  ...
                                  <button
                                    type="button"
                                    class="text-ns-muted hover:text-ns-danger text-xs"
                                    (click)="unsaveResource(r.resourceUrl)"
                                  >
                                    ✕
                                  </button>
                                </div>
                    ...
                    @if (resourceGroupsTotalPages() > 1) {
                      <div class="mt-6 flex items-center justify-between">
                        <button
                          ...
                          (click)="
                            resourceGroupsPage.set(resourceGroupsPage() - 1)
                          "
                        >
                          ← Previous
                        </button>
                        <span class="text-xs text-ns-muted"
                          >Page {{ resourceGroupsPage() + 1 }} of
                          {{ resourceGroupsTotalPages() }}</span
                        >
                        <button
                          type="button"
                          class="rounded-ns border border-ns-border px-4 py-2 text-sm text-ns-muted hover:border-ns-primary hover:text-ns-text disabled:opacity-40 transition-colors"
                  ...
                  @if (results().length === 0) {
                    ...
                    <div class="space-y-3">
                      @for (result of paginatedResults(); track result.id) {
                        <div
                          class="flex items-center justify-between gap-4 rounded-ns-card border border-ns-border bg-ns-card px-5 py-4"
                        >
                          <div>
                            <p class="text-sm font-semibold text-ns-text">
                              {{ result.topCareer }}
                          ...
                          <a
                            ...
                            class="text-xs font-medium text-ns-primary no-underline hover:underline shrink-0"
                            >View result →</a
                          >
                        </div>
                      }
                ...
                <div>
                  @if (analyses().length === 0) {
                    ...
                    <div class="mb-6">
                      ...
                      <p class="text-sm text-ns-muted">
                        Upload your CV or paste your profile and our AI will
                        match you to the best tech careers and tell you how to
                    ...
                    <div class="grid gap-4 sm:grid-cols-3">
                      ...
                      >
                        ...
                        <h3 class="mb-1 text-sm font-semibold text-ns-text">
                          Upload your CV
                        </h3>
                        ...
                        <label class="cursor-pointer">
                          ...
                          />
                          <span
                            class="inline-flex h-8 items-center rounded-ns border border-ns-border bg-ns-canvasSubtle px-3 text-xs font-medium text-ns-text hover:bg-ns-card"
                          >
                            {{
                              selectedFile()
                                ? selectedFile()!.name
                      ...
                      >
                        ...
                        <h3 class="mb-1 text-sm font-semibold text-ns-text">
                          Paste your CV or bio
                        </h3>
                        <p class="mb-3 text-xs text-ns-muted">
                          Any format — LinkedIn about, CV text, or a short bio
                        </p>
                        <textarea
                          class="w-full rounded-ns border border-ns-border bg-ns-bg px-3 py-2 text-sm text-ns-text focus:border-ns-primary focus:outline-none"
                          rows="4"
                          ...
                          [(ngModel)]="cvText"
                        ></textarea>
                        <p class="mt-1 text-right text-xs text-ns-muted">
                          {{ cvText.length }}/10000
                        </p>
                      </div>
                      ...
                      >
                        ...
                        <p class="mb-3 text-xs text-ns-muted">
                          We will extract your public profile
                        </p>
                        <input
                          type="url"
                    ...
                    <div class="space-y-8">
                      @for (
                        ...
                        <div
                          ...
                          class="rounded-ns-lg border border-ns-border bg-ns-card p-6"
                        >
                          <div
                          ...
                          <div class="mb-8 flex flex-col items-center">
                            <div class="relative mb-3">
                              ...
                              >
                                <circle
                                  ...
                                  r="50"
                                  fill="none"
                                  stroke="var(--ns-color-canvas-subtle)"
                                  stroke-width="10"
                                />
                                <circle
                                  cx="60"
                                  ...
                                  fill="none"
                                  [attr.stroke]="
                                    scoreColour(analysis.profileScore)
                                  "
                                  stroke-width="10"
                                  stroke-linecap="round"
                                  [attr.stroke-dasharray]="314"
                                  [attr.stroke-dashoffset]="
                                    314 - (314 * analysis.profileScore) / 100
                                  "
                              ...
                              >
                                <span class="text-3xl font-bold text-ns-text">{{
                            ...
                            <p class="text-base font-semibold text-ns-text">
                              {{ scoreLabel(analysis.profileScore) }}
                            </p>
                            <p
                              class="mt-1 max-w-md text-center text-sm text-ns-muted"
                            >
                              {{ parseFullAnalysis(analysis).summary }}
                            </p>
                          </div>
                          ...
                          <div class="mb-6">
                            <h3
                              class="mb-3 text-base font-semibold text-ns-text"
                            >
                              Your best career matches
                            </h3>
                            <div class="space-y-3">
                              @for (
                                match of asMatches(analysis.topMatches);
                                track match.careerId
                              ) {
                                <div
                                  class="rounded-ns border border-ns-border bg-ns-bg p-3"
                                >
                                  <div
                                  ...
                                  >
                                    ...
                                    <div class="flex items-center gap-2">
                                      <span
                                        ...
                                        [class.bg-ns-successSoft]="
                                          match.tier === 'strong'
                                        "
                                        [class.text-ns-success]="
                                          match.tier === 'strong'
                                        "
                                        [class.bg-ns-warningSoft]="
                                          match.tier === 'good'
                                        "
                                        [class.text-ns-warning]="
                                          match.tier === 'good'
                                        "
                                        [class.bg-ns-canvasSubtle]="
                                          match.tier === 'possible'
                                        "
                                        [class.text-ns-muted]="
                                          match.tier === 'possible'
                                      ...
                                      >
                                      <span
                                        class="text-sm font-bold text-ns-primary"
                                        >{{ match.matchPercentage }}%</span
                                      >
                                    </div>
                                  </div>
                                  <div
                                    class="mb-1 h-1.5 w-full rounded-full bg-ns-canvasSubtle"
                                  >
                                    <div
                                      class="h-1.5 rounded-full bg-ns-primary transition-all"
                                  ...
                                  <a
                                    ...
                                    class="mt-1 inline-block text-xs font-medium text-ns-primary no-underline hover:underline"
                                    >View career path →</a
                                  >
                                </div>
                              }
                          ...
                          <div class="mb-6">
                            <h3
                              class="mb-3 text-base font-semibold text-ns-text"
                            >
                              What your profile does well
                            </h3>
                            <div class="space-y-2">
                              @for (
                                s of asStrengths(analysis.strengths);
                                track s.title
                              ) {
                                <div
                                  class="rounded-ns border-l-4 border-ns-success bg-ns-successSoft px-4 py-3"
                                >
                                  <p class="text-sm font-semibold text-ns-text">
                                    {{ s.title }}
                                  </p>
                          ...
                          <div class="mb-6">
                            ...
                            <div class="space-y-2">
                              @for (g of asGaps(analysis.gaps); track g.title) {
                                <div
                                  class="rounded-ns border-l-4 border-ns-warning bg-ns-warningSoft px-4 py-3"
                                >
                                  <p class="text-sm font-semibold text-ns-text">
                                    {{ g.title }}
                                  </p>
                                  ...
                                  <div class="flex flex-wrap gap-1">
                                    @for (c of g.impactedCareers; track c) {
                                      <span
                                        class="rounded-full bg-ns-card px-2 py-0.5 text-xs text-ns-muted border border-ns-border"
                                        >{{ c }}</span
                                      >
                                    }
                                  </div>
                          ...
                          <div class="mb-6">
                            ...
                            <div class="space-y-2">
                              @for (
                                imp of asImprovements(analysis.improvements);
                                track imp.action
                              ) {
                                <div
                                  class="rounded-ns border border-ns-border bg-ns-bg px-4 py-3"
                                >
                                  <div class="mb-1 flex items-center gap-2">
                                    <span
                                      class="rounded-full px-2 py-0.5 text-xs font-semibold uppercase"
                                      [class.bg-ns-dangerSoft]="
                                        imp.priority === 'high'
                                      "
                                      [class.text-ns-danger]="
                                        imp.priority === 'high'
                                      "
                                      [class.bg-ns-warningSoft]="
                                        imp.priority === 'medium'
                                  ...
                                  <p class="text-xs text-ns-success">
                                    Could improve match by ~{{
                                      imp.impactScore
                                    }}%
                                  </p>
                                </div>
                          ...
                          <div>
                            <h3
                              class="mb-3 text-base font-semibold text-ns-text"
                            >
                              Careers you could pursue now
                            </h3>
                            <div class="grid gap-3 sm:grid-cols-2">
                              @for (
                                rec of asRecommended(
                                  analysis.recommendedCareers
                                );
                                track rec.careerId
                              ) {
                                <div
                                  class="rounded-ns border border-ns-border bg-ns-bg p-4"
                                >
                                  <p
                                    class="mb-1 text-sm font-semibold text-ns-text"
                                  >
                                    {{ rec.careerTitle }}
                                  </p>
                                  <p class="mb-1 text-xs text-ns-muted">
                                  ...
                                  <div
                                    class="mb-2 flex items-center gap-3 text-xs text-ns-muted"
                                  >
                                    <span>⏱ {{ rec.timeToReady }}</span>
                                    <span>Gap: {{ rec.keyGap }}</span>
                                  </div>
                                  <a
                                    [routerLink]="['/careers', rec.careerId]"
                                    class="text-xs font-medium text-ns-primary no-underline hover:underline"
                                    >View roadmap →</a
                                  >
                                </div>
                      ...
                      <div class="flex gap-3">
                        <button
                          type="button"
                          class="rounded-ns border border-ns-border bg-ns-card px-4 py-2 text-sm font-medium text-ns-text hover:bg-ns-canvasSubtle"
                          (click)="resetAndRunNew()"
                        >
                          Analyse new CV
                        </button>
                        <button
                          type="button"
                          class="rounded-ns bg-ns-primary px-4 py-2 text-sm font-medium text-ns-primaryFg hover:bg-ns-primaryHover"
                          (click)="runAnalysis()"
                        >
                          Re-analyse
                        </button>
...
export class ProfilePageComponent implements OnInit {
  ...
  readonly savedResourceGroups = computed(() =>
    Object.entries(this.savedResources()).map(([label, items]) => ({
      label,
      items,
    })),
  );
  ...
  readonly savedCareersTotalPages = computed(() =>
    Math.ceil(
      (this.profile()?.savedCareers.length ?? 0) / this.CAREERS_PER_PAGE,
    ),
  );
  ...
  readonly hasInput = computed(
    () =>
      !!this.selectedFile() ||
      this.cvText.trim().length >= 100 ||
      this.linkedinUrl.trim().length > 0,
  );
  ...
  private loadAll(): void {
    ...
    this.savedService
      .getSavedResources()
      .subscribe((r) => this.savedResources.set(r));
  ...
  toggleVisibility(): void {
    ...
    if (!current) return;
    this.profileService
      .toggleVisibility(!current.isPublic)
  ...
  unsaveCareer(careerId: string): void {
    this.savedService.unsaveCareer(careerId).subscribe(() => {
      ...
      this.profile.set({
        ...p,
        savedCareers: p.savedCareers.filter((c) => c.careerId !== careerId),
      });
    });
  }
  ...
  openSavedResource(resource: SavedResource): void {
    this.externalLink.openExternalLink({
      ...
      platform:
        resource.platform ||
        this.externalLink.extractDomain(resource.resourceUrl),
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  asStrengths(v: any) {
    return v as { title: string; description: string }[];
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  asGaps(v: any) {
    return v as {
